...
import {
  ChefHat, Search, Loader2, Wand2, Plus, CheckCircle, Square, Trash2,
  Globe, Zap, FilePlus, Save as SaveIcon, User, X, LogIn, UserPlus, Mail, Lock,
  ShieldAlert, Moon, Sun, FileText, AlertCircle, RefreshCw, ClipboardType, Terminal, AlignLeft, Edit2,
  Camera, Image as ImageIcon, Link as LinkIcon, Layers, Bug, Key
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    console.log("App Mounted - v2.9.1");
  ...
  useEffect(() => {
    ...
    return (
      ...
      {activeTab === 'recipes' && (
        <div className="card">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-muted font-mono">v2.9.1</p>
          </div>
      ...
      {activeTab === 'logs' && (
        ...
        <div className="flex justify-between items-center mb-4"><div className="font-black text-xs text-primary uppercase">Forensic Trace</div><button onClick={() => window.location.reload()}><RefreshCw size={12}/></button></div>
...
export default App;          console.warn("Failed to parse __firebase_config");
        }
      } else {
        firebaseConfig = __firebase_config;
      }
...
const App = () => {
  ...
  useEffect(() => {
    console.log("App Mounted - v2.9.0");
    // Ensure CSS root variables are set correctly on mount
    const root = document.documentElement;
    if (!root.className) root.className = 'dark';
  }, []);
    ...
    useEffect(() => {
      ...
      return (
        ...
        {isAuthOpen && (
          ...
          <div className="text-center mb-6">
          <h2 className="text-xl font-bold text-primary tracking-tight">WELCOME</h2>
          <p className="text-sm text-muted mt-1">Sign in to sync your pantry</p>
          </div>
        ...
        {activeTab === 'recipes' && (
          ...
          <div className="text-center mb-8">
          <h1 className="text-3xl font-black text-primary tracking-tight">RECIPE MATCH</h1>
          <p className="text-xs text-muted font-mono">v2.9.0</p>
          </div>
          <div className="flex gap-4 mb-6"><Search size={20} className="text-muted"/><input className="input-field" style={{border:'none',background:'none',padding:0}} placeholder="Search recipes..." value={search} onChange={e => setSearch(e.target.value)}/></div>
          <div className="divide-y divide-border/50">
          {(scoredRecipes || []).map(recipe => {
